...
# ── Bulletproof startup for Railway / Render / Gunicorn ──────────
# Uses a file-based lock so even if Gunicorn forks multiple workers,
# only ONE worker starts the scanner thread.
import tempfile, fcntl

_scanner_started = False
_start_lock_file = os.path.join(tempfile.gettempdir(), "madman_scanner.lock")

def _ensure_started():
    global _scanner_started
    if _scanner_started:
        return
    # Try to acquire a file lock — only first process wins
    try:
        ...
        t.start()
        print("🚀 Mad Man Strategy Scanner — threads launched (WSGI/Gunicorn mode)", flush=True)
        # Keep lock open — released when process dies
    except (IOError, OSError):
        # Another worker already started the scanner — that's fine
        _scanner_started = True
        print("ℹ️ Scanner already started by another worker", flush=True)

# Run at module load time — works for Gunicorn --preload AND normal import
